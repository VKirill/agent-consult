...
**Полноценный MCP-сервер для совместных ИИ-консилиумов (Codex, Claude, Anti-Gravity, Mimo, Grok) с профессиональным синтезом ответов через модель Minimax-M3 на платформе OpenRouter.**
...
flowchart TD
    ...
    MCPServer -->|Обычный режим: 5 агентов| Agents
...
* **Параметры**:
  ...
  - `agents` (array, необязательный): Список агентов для опроса (например, `["codex", "claude"]`). По умолчанию `["codex", "claude", "agy", "mimo", "grok"]`.
...
```json
{
  ...
  "agents": {
    ...
    "grok": {
      ...
      "system_prefix": "Ты — агент Grok. Твоя сила в поиске деталей, анализе реального времени, сарказме и прямолинейных практических советах...",
      "reasoning": {
        "enable": false,
        "reasoning_effort": "medium"
      }
...
## 🛡️ Режим аудита безопасности и DevSecOps

Сервер включает специализированный режим **Аудитора безопасности (`security_auditor`)**, ориентированный на статический/динамический анализ кода, сканирование уязвимостей и моделирование угроз:

* **Одиночный запуск с глубокими рассуждениями (High Reasoning)**: При выборе роли `security_auditor` сервер автоматически отключает опрос остальных 4-х агентов и пропускает шаг синтеза. Вместо этого выполняется запуск одного локального агента **Codex** на базе флагманской модели `openai/gpt-5.5` с максимальным уровнем рассуждений (`reasoning_effort: "high"`).
* **Интеграция с SAST/DAST и Supply Chain**: Агенту доступны специальные DevSecOps-инструменты МЦП:
  - **`sentinel`** — единый оркестратор средств безопасности, позволяющий запускать **Trivy** (анализ уязвимостей пакетов/зависимостей), **Semgrep** (Static Application Security Testing — SAST) и **OWASP ZAP** (Dynamic Application Security Testing — DAST).
  - **`skylos`** — специализированный анализатор для поиска жестко захардкоженных секретов, токенов и уязвимых потоков данных (taint analysis) в проектах на JavaScript, TypeScript, Python и Go.
* **Тестирование аудита безопасности**: Вы можете запустить встроенный тест анализа безопасности с помощью команды:
  ```bash
